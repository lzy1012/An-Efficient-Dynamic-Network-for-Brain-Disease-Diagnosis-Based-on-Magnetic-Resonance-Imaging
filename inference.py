...
def plot_confusion_matrix(y_true, y_pred, num_classes):
    sns.set()
    f,ax=plt.subplots()
    C2= confusion_matrix(y_true, y_pred, labels=range(num_classes))
...
parser.add_argument('--checkpoint_path', default='output/GF-mobilenetv3_large_100_patch-size-96_T4_train-stage3/model_best.pth.tar', type=str,
...
def main():
    ...
    if args.eval_mode > 0:
        ...
        train_loader = torch.utils.data.DataLoader(train_set, batch_size=16, num_workers=1, pin_memory=False,
        ...
        for p in range(1, 40):
            ...
            acc_step = dynamic_evaluate(test_logits, test_targets, dynamic_threshold[p-1])
        ...
        preds = torch.argmax(test_logits[-1], dim=1).cpu().numpy()
    ...
    print('准确率：', accuracy_score(labels, preds), '精确率：', precision_score(labels, preds, average='macro'), '召回率：', recall_score(labels, preds, average='macro'))
    plot_confusion_matrix(labels, preds, num_classes=3)
...
def dynamic_evaluate(logits, targets, T):
